...
def parseArgs():
    ...
    parser.add_argument('--path1', nargs='?', default='CDRS/CoNet/Data/books/', type=str,
                        help='Input data path for domain1.')
    parser.add_argument('--path2', nargs='?', default='CDRS/CoNet/Data/elec/', type=str,
...
class CoNetRec(object):
    ...
    def model(self):
        ...
        with tf.variable_scope('Model'):
            ...
            mlp_vector1 = tf.layers.dropout(mlp_vector1, rate=0.2)
            mlp_vector2 = tf.layers.dropout(mlp_vector2, rate=0.2)
    ...
    def train_one_epoch(self, session, init, epoch):
        ...
        try:
            while True:
                ...
                total_map1 += map1
                total_map2 += map2
                if n_batches % self.skip == 0:
                    print("Batch {0} at Epoch {1}: [MAE] {2} and {3}".format(n_batches, epoch, total_map1 / n_batches, total_map2 / n_batches))
