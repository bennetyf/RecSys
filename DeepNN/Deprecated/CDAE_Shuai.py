...
class CDAE():
    ...
    def build_network(self, hidden_neuron = 500, corruption_level = 0):
        ...
        layer_2 = tf.layers.dense(inputs=layer_1, units=hidden_neuron,
                                  kernel_initializer=tf.contrib.layers.xavier_initializer(),
                                  bias_initializer=tf.random_normal_initializer,
                                  activation=tf.nn.sigmoid,
                                  kernel_regularizer=tf.contrib.layers.l2_regularizer(scale=self.reg_rate))
        layer_3 = tf.layers.dense(inputs=layer_2, units=hidden_neuron, activation=tf.nn.sigmoid,
                                  kernel_initializer=tf.contrib.layers.xavier_initializer(),
                                  bias_initializer=tf.random_normal_initializer,
                                  kernel_regularizer=tf.contrib.layers.l2_regularizer(scale=self.reg_rate))
        layer_4 = tf.layers.dense(inputs=layer_3, units=hidden_neuron, activation=tf.nn.sigmoid,
                                  kernel_initializer=tf.contrib.layers.xavier_initializer(),
                                  bias_initializer=tf.random_normal_initializer,
                                  kernel_regularizer=tf.contrib.layers.l2_regularizer(scale=self.reg_rate)) + layer_2

        self.layer_5 = tf.sigmoid( tf.matmul(layer_4, W_prime) + b_prime)

        self.loss = - tf.reduce_sum(self.rating_matrix * tf.log(self.layer_5) + (1 - self.rating_matrix) * tf.log( 1- self.layer_5))\
                                      + self.reg_rate * (tf.nn.l2_loss(W) + tf.nn.l2_loss(W_prime)  + tf.nn.l2_loss(V) + tf.nn.l2_loss(b) + tf.nn.l2_loss(b_prime)) + tf.losses.get_regularization_loss()
    ...
    def test(self):
        self.reconstruction = self.sess.run(self.layer_5,feed_dict={self.corrupted_rating_matrix: self.train_data,
